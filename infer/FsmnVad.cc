...
Ort::Env FsmnVad::env_ = Ort::Env(ORT_LOGGING_LEVEL_FATAL, "");
Ort::SessionOptions FsmnVad::session_options_ = Ort::SessionOptions();

void FsmnVad::init_vad(const std::string &vad_model, int vad_sample_rate, int vad_silence_duration, int vad_max_len,
    ...
    std::string cmvn_path = vad_model.substr(0, vad_model.find_last_of('/'));
    std::ifstream infile(cmvn_path + "/cmvn");
    std::string cmvn_info;
    int i = 0;
    cmvns.resize(2);
    while (getline(infile, cmvn_info)) {
        SplitStringToVectorFloat(cmvn_info, ",", true, &(cmvns[i]));
        i++;
    }
}
...
std::vector<std::vector<float>> &FsmnVad::LfrCmvn(std::vector<std::vector<float>> &vad_feats, int lfr_m, int lfr_n) {
    ...
    for (int i = 0; i < (lfr_m - 1) / 2; i++) {
        vad_feats.emplace(vad_feats.begin(), vad_feats[0]);
    }
    ...
    for (auto &out_feat: out_feats) {
        for (int j = 0; j < cmvns[0].size(); j++) {
            out_feat[j] = (out_feat[j] + cmvns[0][j]) * cmvns[1][j];
        }
    }
    vad_feats = out_feats;
    return vad_feats;
...
void FsmnVad::SplitStringToVectorFloat(const std::string &full, const char *delim, bool omit_empty_strings,
    ...
    while (found != std::string::npos) {
        ...
        if (!omit_empty_strings || (found != start && start != end))
            out->push_back(std::stof(full.substr(start, found - start)));
        start = found + 1;
    }
}
